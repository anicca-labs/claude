#!/usr/bin/env bash
# Doppler-aware MCP server runner for react-web-plugin.
...
# Read doppler.<key> from mcp.config.json. Prefer node (always present in a
# Next.js app, and on the Claude Code web container) and fall back to python3
