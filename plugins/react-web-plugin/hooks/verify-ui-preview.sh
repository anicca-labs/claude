...
# Keep only UI source; drop generated code, route handlers, and tests/stories.
UI=$(printf '%s\n' "$CHANGED" \
  | grep -E '\.tsx$|(^|/)(app|src/app|src/components|components)/.*\.(tsx|css)$' \
  | grep -vE '\.test\.tsx?$|\.spec\.tsx?$|\.stories\.tsx$|(^|/)(generated|orval|__tests__|e2e|\.next)/|(^|/)route\.tsx?$' \
  || true)
[ -n "$UI" ] || exit 0

# Loop guard: block only when this exact set of UI changes hasn't been flagged yet.
GITDIR=$(git rev-parse --git-dir 2>/dev/null) || exit 0
MARKER="$GITDIR/.web-preview-verified"
...
COUNT=$(printf '%s\n' "$UI" | grep -c .)
REASON="$COUNT UI file(s) changed this turn but were not visually verified. Run the preview skill (browser screenshot + dev-server errors + tsc) and confirm the screenshot matches the task before finishing. If the change is genuinely non-visual (types, logic, or config only), you may stop."
