...
function unsupportedInspectResult(tableName, backend) {
    return {
        tableExists: false,
        tableName,
        total: 0,
        byPlatform: {},
        recentTokens: [],
        suggestedMigration: backend === "firebase"
...
async function sendTestPush(options) {
    ...
    if (!res.ok) {
        ...
        const isApnsError = fcmError?.details?.some((d) => d.errorCode === "THIRD_PARTY_AUTH_ERROR");
