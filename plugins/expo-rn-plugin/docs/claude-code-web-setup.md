...
- `.mcp.json` — every command is `${CLAUDE_PLUGIN_ROOT:-.}/bin/mcp-run.sh`. The `:-` fallback is a no-op locally (the plugin is installed, so `CLAUDE_PLUGIN_ROOT` is set) and resolves to the committed `./bin/mcp-run.sh` on the web. Secrets are read by the server from env (or an inner `bash -c "… \"$VAR\""`), never via Claude Code `${VAR}` (which is empty on the web — see [Part 4](#part-4--why-secrets-must-resolve-in-process)). The app copy intentionally **omits** the `expo` and `database` servers — those are the plugin's bundled `dist/` builds (`${CLAUDE_PLUGIN_ROOT}/mcps/…`), which only exist where the plugin is installed. Leaving them in the app file produces a dead `✗ Failed to connect` duplicate locally (the working copy already comes from the plugin) and a non-starter on the web. The plugin owns them; the app file carries only the standalone npx/remote servers.
...
- **`expo` / `database` never run on the web** → expected. They need the plugin's bundled `dist/` builds, which aren't committed in the app; they only run where the plugin is installed, as `plugin:expo-rn-plugin:expo` / `:database`. The app `.mcp.json` deliberately omits them so they don't show up as dead `✗ Failed to connect` duplicates — if you see those, an older app `.mcp.json` still lists them; remove the two entries.
