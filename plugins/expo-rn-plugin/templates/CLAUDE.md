...
- `react-native` / `expo-image` primitives when a Tamagui or `@anicca-labs/` equivalent exists — priority: tamagui → `@anicca-labs/` → project-local → `react-native`; use `YStack`/`XStack` whenever you need spacing, color, or any design token — use `View` only for structural-only wrappers with no styles (e.g. `onLayout` trackers, ref holders)
...
- **Code:** use `@anicca-labs/react-native-splash-view` in `app/_layout.tsx`. Set `animationViewStyle` for Android only: `Platform.OS === "android" ? { width: 288, height: 288, alignSelf: "center" } : undefined` — matches the native splash size (288dp is the Android 12+ icon clip limit; larger values clip without enlarging). iOS is `undefined` because `enableFullScreenImage_legacy` makes the splash full-screen, and Rive fills it identically via `Fit.Contain`.
